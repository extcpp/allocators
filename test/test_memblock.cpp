...
BOOST_AUTO_TEST_CASE(test_deleter_divergent_size)
{
...

	{
		...
		using deleter_t = std::remove_reference_t<decltype(up)>::deleter_type;
		BOOST_CHECK( (std::is_same<deleter_t, alloc::deleter<allocator_t, alloc::deleter_options::divergent_size>>::value) );
...
BOOST_AUTO_TEST_CASE(test_deleter_divsize_local)
{
	...
	allocator_t a;
	{
		...
		using deleter_t = std::remove_reference_t<decltype(up)>::deleter_type;
		BOOST_CHECK( (std::is_same<deleter_t, alloc::deleter<allocator_t, alloc::deleter_options::divergent_size | alloc::deleter_options::local>>::value) );
